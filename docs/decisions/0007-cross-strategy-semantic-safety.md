# ADR 0007: Own semantic safety across the application
...
boundaries, and other application invariants. Those checks do not evaluate the meaning of capture
content or strategy output. User review prevents an interpretation from silently becoming
...
Weavance will treat semantic safety as an application-owned boundary across capture,
interpretation, recommendation, re-entry, and personalization. It is not specific to an LLM
provider.
